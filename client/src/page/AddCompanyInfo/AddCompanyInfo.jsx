...
const AddCompanyInfo = () => {
    ...
    const handleAddCompany = async e => {
        ...
        // cons number = parseFloat(form.price.value)

        const newCompany = { company_name, company_img, company_representative, representative_img, category, membership_number, designation, nature_of_business, office_address, telephone, email, whatsapp, company_profile };


        try {
            const { data } = await axios.post(`${import.meta.env.VITE_API_URL}/add-company`, newCompany);
            console.log(data)
            navigate("/")
        ...
        <div className='relative my-6 p-4 md:p-0'>
            <div className='flex justify-center items-center relative '>
                <section className="container p-8 lg:p-16 mx-auto rounded-md shadow-md bg-opacity-30 bg-gray-400 mt-10">
                    <h2 className="text-3xl font-bold capitalize text-center ">Add Company Info</h2>
                    <form onSubmit={handleAddCompany}>
                        ...
                        <div className="flex justify-center mt-6">
                            <button className="btn px-14 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-cyan-500 rounded-md hover:bg-cyan-700 focus:outline-none ">ADD NOW</button>
                        </div>
                    </form>
                </section>
            </div>
